...
% 定义五组积分区间（单位：秒）
intervals = [0 9.5; 9.5*2 9.5*3; 9.5*4 9.5*5; 9.5*6 9.5*7; 9.5*8 9.5*9];
...
% 定义偏移量 d（单位：秒）
d_values = 0:0.005:40; % 从0秒偏移到4秒，步长0.001秒
...
% 第二组积分区间（单位：秒）
intervals1 = [9.5*10 9.5*11; 9.5*12 9.5*13; 9.5*14 9.5*15; 9.5*16 9.5*17; 9.5*18 9.5*19];
...
% 定义偏移量 d1（单位：秒）
d_values1 = 0:0.005:40; % 从0秒偏移到4秒，步长0.001秒
...
%第三组
% 定义五组积分区间（单位：秒）
intervals2 = [9.5*20 9.5*21; 9.5*22 9.5*23; 9.5*24 9.5*25; 9.5*26 9.5*27; 9.5*28 9.5*29];
...
% 定义偏移量 d（单位：秒）
d_values2 = 0:0.005:40; % 从0秒偏移到4秒，步长0.001秒
...
% 第四组积分区间（单位：秒）
%ddx+
intervals3 = [9.5*30 9.5*31; 9.5*32 9.5*33; 9.5*34 9.5*35; 9.5*36 9.5*37; 9.5*38 9.5*39];
...
% 定义偏移量 d1（单位：秒）
d_values3 = 0:0.005:40; % 从0秒偏移到4秒，步长0.001秒
...
%title('积分值随相对位移变化图');
grid on;
exportgraphics(gcf, 'figure6c.png', 'Resolution', 1200); 
...
figure;
plot(total_integrals, total_integrals1, 'X', 'LineWidth', 0.8);
hold on;
plot(total_integrals2, total_integrals3, 'O', 'LineWidth', 0.8);
hold on;
plot(yy1, yy2);
legend('S_1,S_2 Lissajous plot', 'S_3,S_4 Lissajous plot','Standard circle');
xlabel('S_1,S_3');
ylabel('S_2,S_4');
axis equal;
grid on;
exportgraphics(gcf, 'figure6d.png', 'Resolution', 1200); 
