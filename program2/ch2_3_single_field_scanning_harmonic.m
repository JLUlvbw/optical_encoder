...
% 在图上添加谐波占比文本
%fundamental order
text(freq_d(main_freq_idx)-0.025, main_amplitude_int+1.9,  'Fundamental order','FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'r');


%text(freq_d(main_freq_idx)+0.01, main_amplitude_int, '基频', 'FontSize', 10, 'Color', 'r');
%harmonic order 3rd

text(freq_d(third_int_idx), third_amplitude_int+3.4, ...
    sprintf('3rd: %.2f%%', third_ratio_int), 'FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'b');
text(freq_d(fifth_int_idx), fifth_amplitude_int+2.0, ...
...
grid on;
box on;
legend('Single-field spectrum', 'Fundamental order', 'Harmonic order 3rd', 'Harmonic order 5th', 'FontName', 'Palatino Linotype', 'FontSize', 14 , 'Location', 'northeast');
%legend('频谱', '基频', '三次谐波', '五次谐波', 'Location', 'northeast');
exportgraphics(gcf, 'figure8b.png', 'Resolution', 1200); 
