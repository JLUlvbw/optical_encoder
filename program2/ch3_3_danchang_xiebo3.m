...
% ==================== 积分计算部分 ====================
% 定义积分区间（单位：秒）
interval_length = T/3; % ~6.667秒
ddx = T/7.5;
ddx2 = 2*T/7.5;

% 定义五组积分区间（单位：秒）
intervals = [
             0 interval_length; 9.5*2 9.5*2+interval_length; 9.5*4 9.5*4+interval_length; 9.5*6 9.5*6+interval_length; 9.5*8 9.5*8+interval_length;
             ddx+0 ddx+interval_length; ddx+9.5*2 ddx+9.5*2+interval_length; ddx+9.5*4 ddx+9.5*4+interval_length; ddx+9.5*6 ddx+9.5*6+interval_length; ddx+9.5*8 ddx+9.5*8+interval_length;
             ddx2+0 ddx2+interval_length; ddx2+9.5*2 ddx2+9.5*2+interval_length; ddx2+9.5*4 ddx2+9.5*4+interval_length; ddx2+9.5*6 ddx2+9.5*6+interval_length; ddx2+9.5*8 ddx2+9.5*8+interval_length;
];
%intervals = [0 interval_length; 9.5*2 9.5*2+interval_length; 9.5*4 9.5*4+interval_length; 9.5*6 9.5*6+interval_length; 9.5*8 9.5*8+interval_length];
...
intervals1 = [
    9.5*10 9.5*10+interval_length; 9.5*12 9.5*12+interval_length; 9.5*14 9.5*14+interval_length; 9.5*16 9.5*16+interval_length; 9.5*18 9.5*18+interval_length;
    ddx+9.5*10 ddx+9.5*10+interval_length; ddx+9.5*12 ddx+9.5*12+interval_length; ddx+9.5*14 ddx+9.5*14+interval_length; ddx+9.5*16 ddx+9.5*16+interval_length; ddx+9.5*18 ddx+9.5*18+interval_length;
    ddx2+9.5*10 ddx2+9.5*10+interval_length; ddx2+9.5*12 ddx2+9.5*12+interval_length; ddx2+9.5*14 ddx2+9.5*14+interval_length; ddx2+9.5*16 ddx2+9.5*16+interval_length; ddx2+9.5*18 ddx2+9.5*18+interval_length;
];
...
xlabel('Harmonic order {\it N}','FontName','Palatino Linotype', 'FontSize', 14);     % 纵坐标标签
...
% 在图上添加谐波占比文本
%fundamental order
text(freq_d(main_freq_idx)-0.025, main_amplitude_int+7.5, 'Fundamental order','FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'r');


%text(freq_d(main_freq_idx)+0.01, main_amplitude_int, '基频', 'FontSize', 10, 'Color', 'r');
%harmonic order 3rd

text(freq_d(third_int_idx), third_amplitude_int+5, ...
    sprintf('3rd: %.2f%%', third_ratio_int),'FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'b');
text(freq_d(fifth_int_idx), fifth_amplitude_int+5, ...
...
exportgraphics(gcf, 'figure12b.png', 'Resolution', 1200); 
...
hold on;
plot(yy1, yy2);
legend('S_1,S_2 Lissajous plot','Standard circle');
xlabel('S_1,S_2');
ylabel('S_3,S_4');
axis equal;
grid on;
