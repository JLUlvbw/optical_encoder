...
% 在图上添加谐波占比文本
%fundamental order
text(freq_d(main_freq_idx)-0.025, main_amplitude_int+0.6, 'Fundamental order','FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'r');


%text(freq_d(main_freq_idx)+0.01, main_amplitude_int, '基频', 'FontSize', 10, 'Color', 'r');
%harmonic order 3rd

text(freq_d(third_int_idx), third_amplitude_int+1, ...
    sprintf('3rd: %.2f%%', third_ratio_int),'FontName','Palatino Linotype', 'FontSize', 14, 'Color', 'b');
text(freq_d(fifth_int_idx), fifth_amplitude_int+1, ...
...
exportgraphics(gcf, 'figure9d.png', 'Resolution', 1200); 
...
figure;
plot(total_integrals, total_integrals1,'Color', 'blue','LineWidth', 4);
hold on;
plot(yy1, yy2, 'Color', 'red', 'LineStyle', ':', 'LineWidth', 2.5);
...
axis equal;
grid on;
exportgraphics(gcf, 'figure9c.png', 'Resolution', 1200); 
